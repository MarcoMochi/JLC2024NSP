...
There are 3 main folders, one for the Encodings, one for the Fair Encoding and one containing the Input instances.

Inside the Encodings folder, there are 3 files: one is the Direct encoding (Direct.lp) while the other two represent the encodings for the LBBD methodology, the encoding for the Main problem (MP.lp) and the encoding for the Sub problem (SP.lp).

Inside the Fair folder there is the Direct encoding enriched by the fairness rules (Direct+Fair.lp).
